...
#include <stdlib.h>
#include <time.h>
//#include <gmp.h> //gcc -o executable_file source_file.c -lgmp
#include <math.h>
#include <string.h>
#include <limits.h>
...
//    mpz_out_str(stdout,10,iter_count);
    printf("\n");
}
...
//    mpz_set_ui(iter_count, 0);
    ...
    for(int a = 0; a <= n; a++){
        for(int b = 0; b <= n; b++){
            for(int c = 0; c <= n; c++){
                for(int d = 0; d <= n; d++){
//                    mpz_add_ui(iter_count, iter_count, 1);
                    int image = f(a, b, c, d);
                    int sum = a+b+c+d;
                    if(sum < sums[image]){
                        char buffer[100];
...
int main(int argc, char** argv){ //when compiling use -lgmp flag as notated in the include statements
...
/*
 Max values with minimum input up to 999 (true brute force, taking 13 hours):
